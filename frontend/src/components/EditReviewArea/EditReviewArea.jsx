...
function EditReviewArea(userReview) {
    ...
    const [isRecommended, setIsRecommended] = useState(userReview.props[1].isRecommended);
    ...
    const handleSubmit = async () => {
        const newReview = await dispatch(reviewActions.editReviewThunk({
    ...
    return (
        <div className='text-area-container'>
            ...
            <div>
                <h4 id='text-title'>    
                {editReviewDisable ? <>Edit your review for {`${game?.title}`}</> : <>Your review for {`${game?.title}`}</>}
                </h4>
                {editReviewDisable ? 
            ...
            <div id='user-text-area'>
                ...
                <div id='text-area'>
                    ...
                    {editReviewDisable ? 
                    <textarea id='text' value={text} disabled={!editReviewDisable} onChange={(e) => {
                    ...
                    <p id="recommend-text">Did you recommend this game?</p>
                        ...
                        <div id='button-row'>
                            ...
                            {editReviewDisable ? 
                                ...
                                :
                                <>
                                {isRecommended ? <><FaThumbsUp /> Recommended</> : <><FaThumbsDown /> Not Recommended</>}
                                </>
                            }
                            ...
                            <div id='review-button-container'>
                                {editReviewDisable ?
                                <>
                                <button className={text.length >= 10 && isRecommended !== null && !disabled ? 'review-button' : 'disabled-button'}
                                disabled={disabled} 
                                onClick={handleSubmit}>Post review</button>

                                <button className='review-button'
                                onClick={handleEditExit}>Cancel</button>
                                </>
                                :
                                <>
                                <button className='review-button'
                                onClick={handleEditReviewState}>Edit your review?</button>
                                </>
